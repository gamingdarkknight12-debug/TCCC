...
import { recalculateFantasyPoints, getFantasyStandings } from '../../../lib/fantasyScoring';
...
export async function POST(req) {
  ...
  let fantasyResult = null;
  let fantasyStandings = null;
  if (matchStatus === 'published') {
    await createPredictionPolls(matchDate);

    // Fielding stats (catches/run-outs) entered alongside the scorecard —
    // save them before scoring so this same publish covers everything in
    // one go, no separate follow-up step. Replace-not-append, same as
    // batting/bowling above, so correcting a number on re-publish doesn't
    // double count.
    const fieldingRows = fieldingEntries
      .filter((e) => (e.catches || 0) > 0 || (e.runOuts || 0) > 0)
      .map((e) => ({ match_id: match.id, player_id: e.playerId, catches: e.catches || 0, run_outs: e.runOuts || 0 }));
    await supabaseServer.from('tccc_fielding_stats').delete().eq('match_id', match.id);
    if (fieldingRows.length > 0) {
      await supabaseServer.from('tccc_fielding_stats').insert(fieldingRows);
    }

    const recalc = await recalculateFantasyPoints(match.id);
    if (recalc.teamScores.length > 0) {
      fantasyResult = recalc;
      fantasyStandings = await getFantasyStandings();
    }
  }
  ...
  return NextResponse.json({ matchId: match.id, fantasyResult, fantasyStandings });
}
